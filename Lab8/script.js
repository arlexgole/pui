...
$(document).ready(function() {
	  var animal = JSON.parse(localStorage.getItem('animalSave'));
	  var saved = false;
	  if (animal == null){
	  ...
	  $('#save').click(function(){
	  	if (saved){
	  		// clearing
	  		localStorage.removeItem('animalSave');
	  		$("#save").css("display", "none");
	  		$("#reply").text('Animal has been cleared!');
	  		$("#reply").css("display", "block");
	  	}
	  	else {
	  		localStorage.setItem('animalSave', JSON.stringify(animal));
	  		$("#save").css("display", "none");
	  		$("#reply").text('Animal has been saved!');
	  		$("#reply").css("display", "block");
	  	}
	   });
	});
